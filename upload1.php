    ...
    if ($user_emailid) {
        ...
        if ($uploadOk == 0) {
            ...
            if (move_uploaded_file($_FILES["myfile"]["tmp_name"], $target_file)) {
                ...
                $query = "INSERT INTO uploaded_files (user_emailid, file_name, file_path) VALUES ('$user_emailid', '$file_name', '$file_path')";
                $result = $connect->query($query);

                if ($result) {
                    $response['success'] = "The file " . htmlspecialchars($file_name) . " has been uploaded.";
                    ?>
                    <div class="login-link"><a href="admin dashboard.html">details</a></div>
                    <?php
                } else {
                    $response['error'] = "Error storing file information in the database: " . $connect->error;
                }
            } else {
